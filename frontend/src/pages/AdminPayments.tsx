...
export function AdminPayments() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [invData, payData] = await Promise.all([
        fetchAPI(`/invoices${qs}`),
        fetchAPI(`/invoices/payments${qs}`)
      ]);
      setInvoices(invData || []);
      setPayments(payData || []);
    } catch (err) {
  ...
  const totalOutstanding = invoices
    .filter(inv => inv.bookingId?.status !== 'Cancelled' && inv.orderId?.status !== 'Cancelled')
  ...
  const periodCollections = payments
    .reduce((acc, p) => acc + (p.amount || 0), 0);

  const unpaidCount = invoices.filter(inv => inv.status !== 'Paid' && inv.bookingId?.status !== 'Cancelled' && inv.orderId?.status !== 'Cancelled').length;
    ...
    <>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div className="admin-grid-metrics" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '24px' }}>
          ...
          <div style={{ backgroundColor: 'white', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <span style={{ fontSize: '13px', fontWeight: 600, color: '#64748b' }}>{t('adminPayments.periodCollections', 'Period Collections')}</span>
              <div style={{ backgroundColor: '#eef2ff', padding: '6px', borderRadius: '8px' }}>
                <TrendingUp size={16} color="#4f46e5" />
              </div>
            </div>
            <div style={{ fontSize: '32px', fontWeight: 800, color: '#0f172a', marginBottom: '16px' }}>${periodCollections.toFixed(2)}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', fontWeight: 700, color: '#4f46e5' }}>
              <TrendingUp size={14} /> {t('adminPayments.selectedPeriodTotal', 'Selected period total')}
            </div>
          </div>
        ...
        <div style={{ backgroundColor: 'white', padding: '24px', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column', gap: '16px', position: 'relative', zIndex: 20 }}>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={8} style={{ padding: '2.5rem', textAlign: 'center', color: '#64748b' }}>
                      {t('adminPayments.loading', 'Loading invoices for selected period...')}
                    </td>
                  </tr>
                ) : (() => {
